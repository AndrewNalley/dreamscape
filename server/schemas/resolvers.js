...
const resolvers = {
    Query: {
        ...
        me: async function (parent, args, context) {
            if (context.user) {
                return await User.findOne({ _id: context.user._id }).populate('stories').populate({
                    path: 'stories',
                    populate: 'scenes'
                })
    ...
    Mutation: {
        ...
        createStory: async function (parent, { title }, context) {
            ...
            const story = await Story.create({ title, user: context.user._id, username }) // do same here as below
